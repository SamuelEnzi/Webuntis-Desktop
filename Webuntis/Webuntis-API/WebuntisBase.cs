...
namespace Webuntis_API
{
    public class WebuntisBase
    {
        ...
        public string GetGradeListEndpoint(int personID, string startDate, string endDate) => $"{BaseURL}/WebUntis/api/classreg/grade/gradeList?personId={personID}&startDate={startDate}&endDate={endDate}";
        public string GetTimeTableInfoEndpoint(int personID, string weekStartDate) => $"{BaseURL}/WebUntis/api/public/timetable/weekly/data?elementType=5&elementId={personID}&date={weekStartDate}&formatId=0";
    }
}
